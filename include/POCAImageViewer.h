...
class POCAImageViewer : public TGMainFrame {
	...
	public:	
		...
		POCAImageViewer(const TGWindow *p,UInt_t w,UInt_t h,std::string filename);

		virtual ~POCAImageViewer();
	...
	private:
		...
		TGTextBuffer* fSliderTextBufferXYZ;
		...
		*/
		/*
		MyImage* imageXYList[MAXNIMAGES];
		MyImage* imageXZList[MAXNIMAGES];
		MyImage* imageYZList[MAXNIMAGES];
		MyImage* image3DList[MAXNIMAGES];
		MyImage* currentImageXY;
		MyImage* currentImageXZ;
		MyImage* currentImageYZ;
		MyImage* currentImage3D;
		*/
