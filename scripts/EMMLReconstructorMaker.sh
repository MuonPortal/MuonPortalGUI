...
if [ "$#" -lt 9 ]; then
	...
	echo ""
	echo "*** USAGE ***	"
  echo "$0 --input=[INPUTFILE] --output=[OUTPUTFILE] --config=[CONFIGFILE] --guihost=[GUI_HOSTNAME] --guiport=[GUI_PORT] --guikey=[GUI_KEY] --scanId=[SCAN_ID] --containerId=[CONTAINER_ID] --timeStamp=[TIME_STAMP] [--useMPIVersion] --nprocMPI=[NPROC]"
	echo "**************"
  exit 1
fi
...
if [ "$USE_MPI_VERSION" != "0" ]; then
	CMD="mpirun -np $NPROC $MUPORTAL_GUI_ROOT/EMMLTomographyImaging --input=$INPUTFILE --jobdir=$JOB_DATA_DIR --output=$OUTPUTFILE --config=$CONFIGFILE --guihost=$GUI_HOSTNAME --guiport=$GUI_PORT --guikey=$GUI_KEY --scanId=$SCAN_ID --containerId=$CONTAINER_ID --timeStamp=$TIME_STAMP --useMPIVersion"

else
	CMD="$MUPORTAL_GUI_ROOT/EMMLTomographyImaging --input=$INPUTFILE --jobdir=$JOB_DATA_DIR --output=$OUTPUTFILE --config=$CONFIGFILE --guihost=$GUI_HOSTNAME --guiport=$GUI_PORT --guikey=$GUI_KEY --scanId=$SCAN_ID --containerId=$CONTAINER_ID --timeStamp=$TIME_STAMP"
fi

echo "INFO: EM-ML command: $CMD ..."


shfile="EMMLRun.sh"
echo "INFO: Creating EMML sh script $shfile ..."
(
	echo '#!/bin/sh'
	echo ''
