...
CMD="$MUPORTAL_GUI_ROOT/ACFTomographyImaging --input=$INPUTFILE --jobdir=$JOB_DATA_DIR --output=$OUTPUTFILE --config=$CONFIGFILE --guihost=$GUI_HOSTNAME --guiport=$GUI_PORT --guikey=$GUI_KEY --scanId=$SCAN_ID --containerId=$CONTAINER_ID --timeStamp=$TIME_STAMP"

echo "INFO: ACF command: $CMD ..."


shfile="ACFRun.sh"
echo "INFO: Creating ACF sh script $shfile ..."
(
	echo '#!/bin/sh'
	echo ''
