...
if [ "$IS_LOCAL" = "1" ]; then
	...
	MAKER_CMD="$MUPORTAL_GUI_SCRIPT/POCAReconstructorMaker.sh --input=$INPUTFILE --output=$OUTPUTFILE --config=$CONFIGFILE --guihost=$GUI_HOSTNAME --guiport=$GUI_PORT --guikey=$GUI_KEY --scanId=$SCAN_ID --containerId=$CONTAINER_ID --timeStamp=$TIME_STAMP"
...
if [ "$MAKER_CMD_STATUS" != "0" ]; then
	echo "ERROR: POCAReconstructorMaker script failed on remote host (exit_code=$MAKER_CMD_STATUS)..."
...
RUN_CMD="$JOB_DATA_REMOTE_DIR/POCARun.sh"
RUN_REMOTE_CMD=""
RUN_CMD_STATUS=0
RUN_CMD_PID=-1
...
if [ "$IS_LOCAL" = "1" ]; then
	...
	RUN_REMOTE_CMD="$RUN_CMD >& $JOB_DATA_REMOTE_DIR/POCARun.log < /dev/null & echo "'$!' 
	...
	if [ "$USE_QUEUE_SYSTEM" = "1" ]; then
		...
		RUN_REMOTE_CMD="$HOST_CONNECT_CMD 'nohup $RUN_CMD >& $JOB_DATA_REMOTE_DIR/POCARun.log < /dev/null & echo "'$!'"'" 
...
if [ "$RUN_CMD_STATUS" != "0" ]; then
	echo "ERROR: POCA run failed on remote host (exit_code=$RUN_CMD_STATUS)..."
	exit $RUN_CMD_STATUS
fi


echo $RUN_CMD_PID >> POCA-pid.txt

echo "END POCA SUBMISSION"
