...
if [ "$#" -lt 9 ]; then
	...
	echo ""
	echo "*** USAGE ***	"
  echo "$0 --input=[INPUTFILE] --output=[OUTPUTFILE] --config=[CONFIGFILE] --guihost=[GUI_HOSTNAME] --guiport=[GUI_PORT] --guikey=[GUI_KEY] --scanId=[SCAN_ID] --containerId=[CONTAINER_ID] --timeStamp=[TIME_STAMP] [--local] [--useQueueSystem] [--useMPIVersion] --nprocMPI=[NPROC]"
	echo "**************"
  exit 1
fi
...
echo "USE_MPI_VERSION= $USE_MPI_VERSION"
echo "NPROC= $NPROC"
echo "*****************************"
echo ""
...
if [ "$IS_LOCAL" = "1" ]; then
	...
	##MAKER_CMD="$MUPORTAL_GUI_SCRIPT/EMMLReconstructorMaker.sh --input=$INPUTFILE --output=$OUTPUTFILE --config=$CONFIGFILE --guihost=$GUI_HOSTNAME --guiport=$GUI_PORT --guikey=$GUI_KEY --scanId=$SCAN_ID --containerId=$CONTAINER_ID --timeStamp=$TIME_STAMP --local"
	MAKER_CMD="$MUPORTAL_GUI_SCRIPT/EMMLReconstructorMaker.sh --input=$INPUTFILE --output=$OUTPUTFILE --config=$CONFIGFILE --guihost=$GUI_HOSTNAME --guiport=$GUI_PORT --guikey=$GUI_KEY --scanId=$SCAN_ID --containerId=$CONTAINER_ID --timeStamp=$TIME_STAMP"

	if [ "$USE_MPI_VERSION" != "0" ]; then
		MAKER_CMD="$MAKER_CMD --useMPIVersion --nprocMPI=$NPROC"	
	fi
...
if [ "$MAKER_CMD_STATUS" != "0" ]; then
	echo "ERROR: EMMLReconstructorMaker script failed on remote host (exit_code=$MAKER_CMD_STATUS)..."
...
RUN_CMD="$JOB_DATA_REMOTE_DIR/EMMLRun.sh"
RUN_REMOTE_CMD=""
RUN_CMD_STATUS=0
RUN_CMD_PID=-1
...
if [ "$IS_LOCAL" = "1" ]; then
	...
	RUN_REMOTE_CMD="$RUN_CMD >& $JOB_DATA_REMOTE_DIR/EMMLRun.log < /dev/null & echo "'$!' 
	...
	if [ "$USE_QUEUE_SYSTEM" = "1" ]; then
		...
		RUN_REMOTE_CMD="$HOST_CONNECT_CMD 'nohup $RUN_CMD >& $JOB_DATA_REMOTE_DIR/EMMLRun.log < /dev/null & echo "'$!'"'" 
...
if [ "$RUN_CMD_STATUS" != "0" ]; then
	echo "ERROR: EMML run failed on remote host (exit_code=$RUN_CMD_STATUS)..."
	exit $RUN_CMD_STATUS
fi


echo $RUN_CMD_PID >> EMML-pid.txt

echo "END EMML SUBMISSION"
